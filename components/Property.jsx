...
const Property = ( { property:{ coverPhoto, price, rentFrequency, rooms, title, baths, area, agency, isVerified, externalID  } } ) => (
        <Link href={`property/${externalID}`} passHref>
            <Flex  flexWrap='wrap' w='420px' p='5' pt='0' cursor='pointer'  >
                <Box>
                    <Image objectFit='cover'  src={coverPhoto?.url} width={400} height={260} alt="house"  />
                </Box>
                <Box w='full' >
                    <Flex items='center' pt='1' justify='space-between'  >
                        <Flex items='center'>
                            <Box pr='3' color='green.400'>{isVerified && <GoVerified color='green.400' />}</Box>
                            <Text fontWeight='semibold' fontSize='sm'>birr {millify(price)} {rentFrequency && `/${rentFrequency}`}</Text>
                        </Flex>
                        <Box>
